...
"""
class BLSTM:
    def __init__(self, data, name="", batch_size=128):
        # Note: DataFrame columns are ["vector", "val"], so:
        # data.iloc[:, 0] = vectors, data.iloc[:, 1] = labels (val)
        vectors = np.stack(data.iloc[:, 0].values)  # vector column
        labels = data.iloc[:, 1].values              # val column (labels)
    ...
    """
    def train(self):
        self.model.fit(self.X_train, self.y_train, batch_size=self.batch_size, epochs=128, class_weight=self.class_weight)
        self.model.save_weights(self.name + "_model.weights.h5")
    ...
    """
    def test(self):
        self.model.load_weights(self.name + "_model.weights.h5")
